...
public class UserService {
	public Integer register(User user) {
		try {
			...
			if (UserDao.hasUser(user, databaseDao)) {
				return 0;// 失败，用户已存在
			} else if (UserDao.hasStringValue("name", user.getName()) == 1) {
				return -1;
			} else if (UserDao.hasStringValue("email", user.getEmail()) == 1) {
				return -10;
			} else {// 没有同名用户，可以注册
	...
	public Integer findPasswordByEmail(User user, Integer rand) throws Exception {// 返回值：1成功发送邮件，-1发送邮件失败，-2邮箱未注册过
		UserDao userDao = new UserDao();
		Integer result = 0;
